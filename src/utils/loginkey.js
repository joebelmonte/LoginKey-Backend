...
function generateLoginKey({ partnerId, partnerUserId, apiKey, timeout }) {
/*
CryptoJS v3.1.2
code.google.com/p/crypto-js
(c) 2009-2013 by Jeff Mott. All rights reserved.
...
map();
    ...
    var hmacb64replaced = hmacb64.replace(/\+/g, '-').replace(/\//g, '_')
    var loginkey =
      "$" +
      version +
      "$" +
      expires +
      "$" +
      hmacb64.replace(/\+/g, '-').replace(/\//g, '_').substr(0, 43);
    return loginkey;
  }

  function generateLoginKeys (groups) {
    for (var i = 0; i < groups.length; i++) {
      groups[i].loginKey = generateLoginKey(groups[i])
    }
  }
